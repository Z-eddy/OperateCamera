...
void main()
{
    ...
    try
    {
        ...
        //设置视频编码类型和帧率，有三种选择
        // 帧率必须小于等于相机成像帧率
		cvVideoCreator.open(videoFileName, CAP_OPENCV_MJPEG, 10, frameSize);
...

        {
            ...
            if (ptrGrabResult->GrabSucceeded())
            {
                ...
                openCvImage = cv::Mat(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8UC3, (uint8_t *) pylonImage.GetBuffer());// ptrGrabResult.GetBuffer不行吗
                ...
                namedWindow("OpenCV Display Window"); // other options: CV_AUTOSIZE, CV_FREERATIO
                //显示及时影像.
